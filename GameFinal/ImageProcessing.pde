...
public void setup() {
  quad = new QuadGraph();
  projector = new TwoDThreeD(width, height);
...
public void draw() { 
  ...
  if (cam.available() == true) {
    cam.read();
      ...
      intersections = getIntersections(quad.drawQuads(lines));
...
PVector getRotation(){
  ...
  PVector rot = projector.get3DRotations(sortedCorners);
  return rot;
}

//TODO: how to connect the corners to the rotation of the plate? this function returns the corners sorted
List<PVector> getCorners(){
    ...
    /**Can't do that*/
      ...
      /**edgeImg = sobel(intensityThresholding(convolute(HSBMapping(img), gaussianKernel, false)));
      lines = hough(edgeImg, nLines);
      List<PVector> intersections = getIntersections(quad.drawQuads(lines));*/
      List<PVector> sortedCorners = sortCorners(intersections);
